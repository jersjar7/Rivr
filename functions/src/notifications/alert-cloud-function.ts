...
// Simplified version replacing the complex existing implementation
...
const SCALE_FACTOR = parseFloat(process.env.NOTIFICATION_SCALE_FACTOR || '1');
const CHECK_FREQUENCY = process.env.NOTIFICATION_CHECK_FREQUENCY_MINUTES || '360';
...
export const checkFlowNotifications = functions.pubsub
  ...
  .onRun(async (context) => {
    ...
    console.log(`🌊 Starting simple notification check (scale factor: ${SCALE_FACTOR})...`);
    
    try {
      // Step 1: Get all users with notifications enabled
      const enabledUsers = await getEnabledUsers();
      console.log(`👥 Found ${enabledUsers.length} users with notifications enabled`);
 ...
 */
async function getEnabledUsers(): Promise<Array<{userId: string, fcmToken: string}>> {
 ...
 */
async function checkUserFavorites(user: {userId: string, fcmToken: string}): Promise<number> {
  ...
  try {
    ...
    console.log(`📋 User ${user.userId} has ${favoritesSnapshot.docs.length} favorites`);
...
async function checkReachForNotification(
  ...
  try {
    ...
    const exceedsThreshold = checkScaledThresholds(forecastData, returnPeriod);
    
    if (exceedsThreshold.shouldNotify) {
      await sendSimpleNotification(user, reachId, exceedsThreshold, forecastData.reachName);
      return true;
    }
...
async function getForecastData(reachId: string): Promise<{
  ...
  if (data?.shortRange?.forecasts) {
    const shortRangeMax = Math.max(...data.shortRange.forecasts.map((f: any) => f.value || 0));
  ...
  if (data?.mediumRange?.forecasts) {
    const mediumRangeMax = Math.max(...data.mediumRange.forecasts.map((f: any) => f.value || 0));
...
async function getReturnPeriodData(reachId: string): Promise<{
  ...
  const returnPeriodDoc = await db.collection('returnPeriodCache').doc(reachId).get();
...
async function sendSimpleNotification(
  ...
  try {
    ...
    console.log(`📱 Notification sent to user ${user.userId} for reach ${reachId}`);
...
// No test function needed - using 1-minute frequency for development testing