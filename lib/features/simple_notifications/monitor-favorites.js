...
/**
 * Monitor rivers for a specific user (UPDATED to include dummy rivers)
 */
async function monitorUserRivers(userPreferences) {
  ...
  // Monitor regular favorite rivers
  ...
  // Monitor dummy rivers with active forecasts
  try {
    const dummyRiverAlerts = await checkDummyRiversForAlerts(userPreferences);
    alerts.push(...dummyRiverAlerts);
  } catch (error) {
    console.warn(`⚠️ Error checking dummy rivers:`, error);
  }
...
/**
 * Check dummy rivers for flow alerts (NEW FUNCTION)
 */
async function checkDummyRiversForAlerts(userPreferences) {
  try {
    console.log(`🧪 Checking dummy rivers for alerts for user ${userPreferences.userId}`);
    
    // Get all dummy rivers with forecasts for this user
    const dummyRiversWithForecasts = await getDummyRiversWithForecasts(userPreferences.userId);
    
    if (dummyRiversWithForecasts.length === 0) {
      console.log(`ℹ️ No dummy rivers with forecasts found for user ${userPreferences.userId}`);
      return [];
    }

    console.log(`🧪 Found ${dummyRiversWithForecasts.length} dummy rivers with forecasts`);

    const alerts = [];

    for (const dummyRiverData of dummyRiversWithForecasts) {
      try {
        const riverAlerts = await checkDummyRiverForAlerts(dummyRiverData, userPreferences);
        alerts.push(...riverAlerts);
      } catch (error) {
        console.warn(`⚠️ Error checking dummy river ${dummyRiverData.riverId}:`, error);
      }
    }

    return alerts;
  } catch (error) {
    console.error(`❌ Error checking dummy rivers for user ${userPreferences.userId}:`, error);
    return [];
  }
}

/**
 * Get dummy rivers with active forecasts for a user (NEW FUNCTION)
 */
async function getDummyRiversWithForecasts(userId) {
  try {
    // Check for dummy river forecasts in the dummyRiverForecasts collection
    // This collection would be populated by the frontend when forecasts are generated
    const forecastsSnapshot = await db
      .collection('dummyRiverForecasts')
      .where('userId', '==', userId)
      .get();

    const dummyRiversWithForecasts = [];

    for (const doc of forecastsSnapshot.docs) {
      const forecastData = doc.data();
      
      // Check if forecast is recent (within last 24 hours)
      const lastUpdated = forecastData.updatedAt?.toDate() || forecastData.lastUpdated?.toDate();
      if (lastUpdated && isCacheValid({ lastUpdated }, 24 * 60 * 60 * 1000)) {
        dummyRiversWithForecasts.push({
          id: doc.id,
          ...forecastData
        });
      }
    ...
    return dummyRiversWithForecasts;
  } catch (error) {
    console.error('❌ Error getting dummy rivers with forecasts:', error);
    return [];
  }
}

/**
 * Check a specific dummy river for flow alerts (NEW FUNCTION)
 */
async function checkDummyRiverForAlerts(dummyRiverData, userPreferences) {
  try {
    console.log(`🧪 Checking dummy river ${dummyRiverData.riverName} for alerts`);

    // Get dummy river return periods
    const returnPeriods = {
      riverId: dummyRiverData.riverId,
      periods: dummyRiverData.returnPeriods || {},
      unit: dummyRiverData.unit || 'cfs'
    };

    if (Object.keys(returnPeriods.periods).length === 0) {
      console.warn(`⚠️ No return periods available for dummy river ${dummyRiverData.riverId}`);
      return [];
    }

    // Prepare flow data in the same format as real rivers
    const flowData = {
      riverId: dummyRiverData.riverId,
      shortRangeForecasts: dummyRiverData.shortRangeForecasts || [],
      mediumRangeForecasts: dummyRiverData.mediumRangeForecasts || [],
      lastUpdated: dummyRiverData.updatedAt?.toDate() || new Date()
    };

    // Convert forecast format to match expected structure
    const convertedFlowData = {
      ...flowData,
      shortRangeForecasts: flowData.shortRangeForecasts.map(forecast => ({
        flow: forecast.flowValue,
        unit: forecast.unit,
        validTime: forecast.timestamp?.toDate() || new Date(forecast.timestamp),
        range: 'short'
      })),
      mediumRangeForecasts: flowData.mediumRangeForecasts.map(forecast => ({
        flow: forecast.flowValue,
        unit: forecast.unit,
        validTime: forecast.timestamp?.toDate() || new Date(forecast.timestamp),
        range: 'medium'
    ...
    const alerts = checkForecastsAgainstReturnPeriods(
      ...
      returnPeriods,
      userPreferences,
      dummyRiverData.riverName
    );

    // Mark alerts as coming from dummy rivers
    alerts.forEach(alert => {
      alert.isDummyRiver = true;
      alert.dummyRiverId = dummyRiverData.riverId;
    });

    return alerts;
  } catch (error) {
    console.error(`❌ Error checking dummy river ${dummyRiverData.riverId}:`, error);
    return [];
  }
}
...
/**
 * Get flow forecast data (UPDATED to include dummy rivers)
 */
async function getFlowForecastData(riverId) {
  try {
    // First check if this is a dummy river forecast
    const dummyForecast = await getDummyRiverForecastData(riverId);
    if (dummyForecast) {
      console.log(`🧪 Using dummy river forecast data for ${riverId}`);
      return dummyForecast;
    }

    // Original logic for real rivers
    // First try cache
...
/**
 * Get dummy river forecast data (NEW FUNCTION)
 */
async function getDummyRiverForecastData(riverId) {
  try {
    // Check the dummyRiverForecasts collection
    const doc = await db.collection('dummyRiverForecasts').doc(riverId).get();
    
    if (doc.exists) {
      const data = doc.data();
      
      // Check if forecast is recent
      const lastUpdated = data.updatedAt?.toDate() || data.lastUpdated?.toDate();
      if (lastUpdated && isCacheValid({ lastUpdated }, 24 * 60 * 60 * 1000)) {
        
        // Convert dummy river forecast format to match expected structure
        return {
          riverId,
          shortRangeForecasts: (data.shortRangeForecasts || []).map(forecast => ({
            flow: forecast.flowValue,
            unit: forecast.unit,
            validTime: forecast.timestamp?.toDate() || new Date(forecast.timestamp),
            range: 'short'
          })),
          mediumRangeForecasts: (data.mediumRangeForecasts || []).map(forecast => ({
            flow: forecast.flowValue,
            unit: forecast.unit,
            validTime: forecast.timestamp?.toDate() || new Date(forecast.timestamp),
            range: 'medium'
    ...
    console.error(`❌ Error getting dummy river forecast data for ${riverId}:`, error);
    return null;
  }
}
...
/**
 * Send FCM notification for alert (UPDATED to handle dummy rivers)
 */
async function sendFCMNotification(alert) {
  try {
    ...
    // Modify title for dummy river alerts
    const titlePrefix = alert.isDummyRiver ? '🧪 Test Alert: ' : '';
    const title = `${titlePrefix}${getSeverityDisplayName(alert.severity)} Flow Alert: ${alert.riverName}`;
    ...
    const message = {
      ...
      data: {
        ...
        isDummyRiver: alert.isDummyRiver ? 'true' : 'false',
        dummyRiverId: alert.dummyRiverId || ''
      },
      android: {
        priority: 'high',
        notification: {
    ...
    console.log(`📱 ${alert.isDummyRiver ? 'Dummy river' : 'Real'} notification sent for ${alert.riverName}`);
    return true;
  } catch (error) {
